...
## Note
This guide has been updated to include Prometheus, PostgreSQL client, and DDS support.
While CentOS 7 is end-of-life, this guide and the automated RPM script should work on RHEL, AlmaLinux, Rocky Linux, and Fedora.

## Automated Installation Script (RPM-based)

As an alternative to following the manual steps below, you can use the automated installation script for RPM-based distributions:
```bash
wget https://raw.githubusercontent.com/FIWARE/context.Orion-LD/develop/doc/manuals-ld/install-environment-rpm.sh
chmod +x install-environment-rpm.sh
./install-environment-rpm.sh                        # Basic installation
./install-environment-rpm.sh --with-dds             # With DDS support
./install-environment-rpm.sh --with-tests           # With test dependencies
./install-environment-rpm.sh --with-dds --with-tests  # With both
```
This script supports RHEL, CentOS, AlmaLinux, Rocky Linux, and Fedora.

**Note:** This script was generated by Claude Opus 4.5 (Anthropic) and has not been thoroughly tested. Use at your own risk.
