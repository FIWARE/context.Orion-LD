...
Ubuntu and Debian are related, so the automated installation script for Ubuntu should also work on Debian.

## Automated Installation Script

As an alternative to following the manual steps below, you can try the automated installation script (designed for Ubuntu but may work on Debian):
```bash
wget https://raw.githubusercontent.com/FIWARE/context.Orion-LD/develop/doc/manuals-ld/install-environment.sh
chmod +x install-environment.sh
./install-environment.sh                        # Basic installation
./install-environment.sh --with-dds             # With DDS support
./install-environment.sh --with-tests           # With test dependencies
./install-environment.sh --with-dds --with-tests  # With both
```
