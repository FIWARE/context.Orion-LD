...
static std::string entityIdId(const std::string& path, const std::string& value, ParseData* parseDataP)
{
  ...
  KT_T(KtLegacy, "Set 'id' to '%s' for an entity", parseDataP->qcrs.cerP->contextElement.entityId.id.c_str());
...
static std::string entityIdType(const std::string& path, const std::string& value, ParseData* parseDataP)
{
  ...
  KT_T(KtLegacy, "Set 'type' to '%s' for an entity", parseDataP->qcrs.cerP->contextElement.entityId.type.c_str());
...
static std::string statusCodeCode(const std::string& path, const std::string& value, ParseData* parseDataP)
{
  ...
  KT_T(KtLegacy, "Got a statusCode code: %d", (int) parseDataP->qcrs.cerP->statusCode.code);
  return "OK";
}
...
{
  KT_T(KtLegacy, "Got a errorCode code: '%s'", value.c_str());
  parseDataP->qcrs.res.errorCode.code = (HttpStatusCode) atoi(value.c_str());
  KT_T(KtLegacy, "Got a errorCode code: %d", (int) parseDataP->qcrs.res.errorCode.code);
  return "OK";
}
...
{
  KT_T(KtLegacy, "Got a errorCode reasonPhrase: '%s'", value.c_str());
...
{
  KT_T(KtLegacy, "Got a errorCode details: '%s'", value.c_str());
...
std::string jsonQcrsCheck(ParseData* reqDataP, ConnectionInfo* ciP)
{
  ...
  return reqDataP->qcrs.res.check(orionldState.apiVersion, asJsonObject, reqDataP->errorString);
}
