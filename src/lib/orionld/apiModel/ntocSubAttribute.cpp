...
// ntocSubAttribute -
//
void ntocSubAttribute(KjNode* saP, const char* lang, bool sysAttrs)
{
  ...
  if ((lang != NULL) && (typeP != NULL) && (strcmp(typeP->value.s, "LanguageProperty") == 0))
  {
    ...
    langFixNormalized(saP, typeP, languageMapP, lang);  // the concise part comes later
  }

  // 3. Remove the attribute type
  if (typeP != NULL)
    kjChildRemove(saP, typeP);

  // 4. If "value" is the only child left - Simplified
  KjNode* valueP     = kjLookup(saP, "value");
  int     attrFields = kjChildCount(saP);

  if ((valueP != NULL) && (attrFields == 1))  // Simplified
  {
    saP->value     = valueP->value;
    saP->type      = valueP->type;
    saP->lastChild = valueP->lastChild;
