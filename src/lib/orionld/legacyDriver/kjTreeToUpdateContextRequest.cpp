...
#include "orionld/legacyDriver/kjTreeToUpdateContextRequest.h"   // Own interface
...
void kjTreeToUpdateContextRequest(UpdateContextRequest* ucrP, KjNode* treeP, KjNode* errorsArrayP, KjNode* idTypeAndCreDateFromDb)
{
  ...
  while (entityP != NULL)
  {
    ...
    if (entityType == NULL)
    {
      entityErrorPush(errorsArrayP, entityId, OrionldBadRequestData, "unable to find entity::type", "not in payload nor in database", 400);
      entityP = next;
      continue;
    }
