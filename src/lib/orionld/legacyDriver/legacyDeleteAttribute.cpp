...
//
static bool deleteAttributeDatasetId(const char* entityId, const char* attrNameExpanded, const char* attrNameExpandedEq, const char* datasetId)
{
  char fieldPath[512];
...
//
bool legacyDeleteAttribute(void)
{
  ...
  if (orionldState.uriParams.datasetId != NULL)
  {
    return deleteAttributeDatasetId(entityId, attrNameExpanded, attrNameExpandedEq, orionldState.uriParams.datasetId);
  }
  else if (orionldState.uriParams.deleteAll == true)
  {
    if (deleteAttributeDatasetId(entityId, attrName, attrNameExpandedEq, NULL) == false)
      return false;
  }
